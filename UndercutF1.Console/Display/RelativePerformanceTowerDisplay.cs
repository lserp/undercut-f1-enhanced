...
public class RelativePerformanceTowerDisplay(
    State state,
...
{
    public Screen Screen => Screen.RelativePerformanceTower;
    ...
    private IRenderable GetRaceTimingTower()
    {
        ...
        table
            .AddColumns(
                ...
                new TableColumn("Int %") { Width = 8, Alignment = Justify.Right },
        ...
        // Get leader's last lap time for percentage calculation
        // We need a reference time. Ideally Leader's Last Lap.
        // If not available, Leader's Best Lap.
        // If not available, maybe the session is just starting.
        var leaderLine = lines.Values.FirstOrDefault(x => x.Position == "1") ?? lines.Values.FirstOrDefault();
        
        var leaderLastLapSeconds = leaderLine?.LastLapTime?.ToTimeSpan()?.TotalSeconds ?? 0;
        if (leaderLastLapSeconds <= 0)
        {
            leaderLastLapSeconds = leaderLine?.BestLapTime?.ToTimeSpan()?.TotalSeconds ?? 0;
        }
        
        // DEBUG: Force a reference time to test if display works
        if (leaderLastLapSeconds <= 0) leaderLastLapSeconds = 90.0;
        ...
        foreach (var (driverNumber, line) in lines)
        {
            ...
            // Calculate Gap %
            var gapToLeaderPercent = GetPercentageString(line.GapToLeader, leaderLastLapSeconds);
            ...
            table.AddRow(
                ...
                new Text(
                    gapToLeaderPercent.ToFixedWidth(8),
                ...
                position.Status == PositionDataPoint.PositionData.Entry.DriverStatus.OffTrack
                    ...
                    : new Text(
                        $"{(car?.Channels.Drs >= 8 ? "•" : "")}{intervalPercent}".ToFixedWidth(
                            8
                        ),
    ...
    private string GetPercentageString(string? timeString, double leaderLastLapSeconds)
    {
        if (string.IsNullOrWhiteSpace(timeString))
            return "";

        if (leaderLastLapSeconds <= 0)
            return timeString; // Fallback if leader time is invalid

        // Handle "LAP" or "L" (lapped cars)
        if (timeString.Contains("L", StringComparison.OrdinalIgnoreCase) || timeString.Contains("LAP", StringComparison.OrdinalIgnoreCase))
            return timeString;

        // Handle "INTERVAL" (leader)
        if (timeString.Equals("INTERVAL", StringComparison.OrdinalIgnoreCase))
            return "-";

        // Try to parse time
        // Formats can be "+1.234", "1.234", "1:23.456"
        var cleanTimeString = timeString.Replace("+", "").Trim();
        
        // DEBUG: Log what we are trying to parse
        // System.Diagnostics.Debug.WriteLine($"Parsing: '{cleanTimeString}' against LeaderLap: {leaderLastLapSeconds}");

        if (TimeSpan.TryParseExact(cleanTimeString, ["s\\.fff", "m\\:ss\\.fff"], CultureInfo.InvariantCulture, out var timeSpan))
        {
            var pct = (timeSpan.TotalSeconds / leaderLastLapSeconds) * 100;
            return $"{pct:F1}%";
        }
        
        // Try parsing as double directly if TimeSpan fails (sometimes it's just seconds)
        if (double.TryParse(cleanTimeString, NumberStyles.Any, CultureInfo.InvariantCulture, out var seconds))
        {
             var pct = (seconds / leaderLastLapSeconds) * 100;
             return $"{pct:F1}%";
        }

        // If we reach here, parsing failed. Let's return the original string but maybe with a marker?
        // Or maybe the format is different?
        // Let's try a more lenient TimeSpan parse
        if (TimeSpan.TryParse(cleanTimeString, CultureInfo.InvariantCulture, out var looseTimeSpan))
        {
            var pct = (looseTimeSpan.TotalSeconds / leaderLastLapSeconds) * 100;
            return $"{pct:F1}%";
        }
    ...
    {
        // Reuse existing non-race logic as relative performance is mostly relevant for race gaps
